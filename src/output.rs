...
impl Render for Task {
    fn render(&self, w: &mut dyn Write) -> Result<()> {
        ...
        let cancelled = state == TaskState::Cancelled;

        let state_style = if cancelled {
            style(state.as_ref()).dim()
        } else {
            style(state.as_ref()).yellow()
        };

        let id_style = if cancelled {
...
mod tests {
    ...
    use crate::models::{Contract, Outcome, Priority};
    use jiff::Timestamp;
    ...
    fn task() -> Task {
        ...
        Task::new(
            TaskId::from("t_abc123".to_string()),
            GoalId::from("g_xyz789".to_string()),
            None,
            None,
            "test task".to_string(),
            Priority::default(),
    ...
    fn render_to_string(item: &dyn Render) -> String {
        let mut buf = Vec::new();
        item.render(&mut buf).unwrap();
    ...
    fn render_includes_contract_fields(mut task: Task) {
        task.set_contract(Contract::new(
    ...
    fn render_includes_blocked_by(mut task: Task) {
        task.set_derived_state(TaskState::Blocked);
        task.set_blocked_by(vec![TaskId::from("t_other".to_string())]);
        let output = render_to_string(&task);
    ...
    fn render_includes_result(mut task: Task) {
        task.set_derived_state(TaskState::Completed);
        task.set_result(Outcome::new(
            "all good".to_string(),
